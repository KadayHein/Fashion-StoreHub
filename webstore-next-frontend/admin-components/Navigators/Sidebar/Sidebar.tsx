...
import navItems from '@/admin-components/data/admin_panel/nav-items';
import NavBox from './NavBox';
import { URL_WEBLOGO } from '@/service/routeHandler';
import { useAdminContext } from '@/app/[locale]/fashion/sysadmin/layout';

type SidebarProps = {
  handleDrawerClose?: () => void
}

export default function Sidebar({ handleDrawerClose }: SidebarProps) {
  const barwidth = 270;
  const { selectedNavItems, setSelectedNavItems } = useAdminContext()

  const isSelected = (title: string) => selectedNavItems.includes(title)

  const [openNavIndex, setOpenNavIndex] =
        ...
        >
          {navItems.map((navItem, index) => (
            <NavBox key={index}
              ...
              navItem={navItem} handleDrawerClose={handleDrawerClose}
